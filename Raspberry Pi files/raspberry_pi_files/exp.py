    ...
    #     # Calculate the center of the contour
	    ...
	    if M['m00'] != 0:
		    ...
		    if cv2.contourArea(contour) > 100:  # Adjust the size threshold as needed
			    detected_coordinates[cnt].append((cx, cy))
...
#        detected_coordinates[cnt].append((i, i))
 
#    detected_coordinates.append([])
#    for i in range (100,200):
#        detected_coordinates[1].append((i, i+i))
 
#    detected_coordinates.append([])
#    for i in range (5,20):
#        detected_coordinates[2].append((i, i*i))
...
#for x in range(len(detected_coordinates)):
    #for y in range(len(detected_coordinates[x])):
        #print(detected_coordinates[x][y]),
    #print("\n")
 
for i in range(len(detected_coordinates)):
    for j in range(1,len(detected_coordinates[i])):
        print(i," ",j," ",detected_coordinates[i][j-1]," ",detected_coordinates[i][j])
        cv2.line(img,detected_coordinates[i][j-1], detected_coordinates[i][j], (0, 255, 0), 2)
